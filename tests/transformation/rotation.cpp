...
bool test_rotating_a_point_around_x_axis() {
    ...
    mn::Point p = mn::make_point(0.0, 1.0, 0.0);

    double HALF_SQRT_OF_2 = std::sqrt(2) / 2;

    if (!mn::epsilon_equal(half_quarter * p, mn::make_point(0.0, HALF_SQRT_OF_2, HALF_SQRT_OF_2))) return false;
    if (!mn::epsilon_equal(full_quarter * p, mn::make_point(0.0, 0.0, 1.0))) return false;
...
bool test_inverse_of_a_x_rotation_rotates_in_the_opposite_direction() {
    ...
    if (!mn::epsilon_equal(mn::inverse(half_quarter) * p, mn::make_point(0.0, HALF_SQRT_OF_2, -HALF_SQRT_OF_2)))
        return false;
    return true;
}
...
bool test_rotating_a_point_around_y_axis() {
    ...
    mn::Point p = mn::make_point(0.0, 0.0, 1.0);

    double HALF_SQRT_OF_2 = std::sqrt(2) / 2;;

    if (!mn::epsilon_equal(half_quarter * p, mn::make_point(HALF_SQRT_OF_2, 0.0, HALF_SQRT_OF_2))) return false;
    if (!mn::epsilon_equal(full_quarter * p, mn::make_point(1.0, 0.0, 0.0))) return false;
...
bool test_rotating_a_point_around_z_axis() {
    ...
    if (!mn::epsilon_equal(half_quarter * p, mn::make_point(-HALF_SQRT_OF_2, HALF_SQRT_OF_2, 0.0))) return false;
    if (!mn::epsilon_equal(full_quarter * p, mn::make_point(-1.0, 0.0, 0.0))) return false;
