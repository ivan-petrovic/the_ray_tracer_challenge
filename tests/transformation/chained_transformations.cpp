...
bool test_individual_transformations_applied_in_sequence() {
    ...
    mn::Point p = mn::point(1.0, 0.0, 1.0);

    // Apply rotation first
    mn::Point p2 = R * p;
    if (!mn::epsilon_equal(p2, mn::point(1.0, -1.0, 0.0))) return false;

    // Then apply scaling
    mn::Point p3 = S * p2;
    if (!mn::epsilon_equal(p3, mn::point(5.0, -5.0, 0.0))) return false;

    // Then apply translation
    mn::Point p4 = T * p3;
    if (!mn::epsilon_equal(p4, mn::point(15.0, 0.0, 7.0))) return false;
...
bool test_chained_transformations_must_be_applied_in_reverse_order() {
    ...
    if (!mn::epsilon_equal(M * p, mn::point(15.0, 0.0, 7.0))) return false;
